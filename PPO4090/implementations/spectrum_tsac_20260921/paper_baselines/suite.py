...
NAMES = {"fixed": "Fixed", "greedy": "Greedy", "random": "Random", "mlp_sac": "MLP-SAC",
         ...
         "horizon_primary": "Spectrum T-SAC主参考η=0.5", "horizon_supplemental": "Spectrum T-SAC验证选择补充参考"}
DB_NAME = "运行记录.sqlite"
...
def _node_output_root():
    from project_paths import SPECTRUM_OUTPUT_DIR
    return SPECTRUM_OUTPUT_DIR
    ...
    for row in plan["experiments"]:
        ...
        try:
            with log_path.open("w", encoding="utf-8") as log:
                process = subprocess.Popen([sys.executable, "-m", "implementations.spectrum_tsac_20260921.paper_baselines.train",
...
def _evaluate(request, result_dir, timeout):
    ...
    with request_path.with_suffix(".log").open("w", encoding="utf-8") as log:
        process = subprocess.Popen([sys.executable, "-m", "implementations.spectrum_tsac_20260921.paper_baselines.suite",
