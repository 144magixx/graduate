...
def prepare_dataset(config, manifest_path=None):
    ...
    selected = Path(manifest_path) if manifest_path else SPECTRUM_DATA_DIR / "训练场景清单.json"
...
def run_training(config, mode="train", dataset=None, resume=None, initialize_from=None):
    if resume and initialize_from:
        raise ValueError("精确恢复与仅权重初始化不可同时使用")
    ...
    initialization = None
    if initialize_from:
        from .initialization import initialize_from_checkpoint
        initialization = initialize_from_checkpoint(agent, config, data_manifest, initialize_from)
    if payload:
    ...
    manifest.update(resume_env_step=counters["env_step"], resume_update_step=counters["update_step"],
                    ...
                    warmup_collection="initialized_policy" if initialization else "uniform_legal",
                    trace_detail="basic_all保留全部动作/资源/逐束速率满足度；逐槽SINR与干扰明细留空，终局交接独立提供")
    ...
    try:
        for episode_offset in range(config.train.episodes):
            ...
            while not obs["terminal"]:
                ...
                if counters["env_step"] < config.train.warmup_steps and not initialization:
...
def main():
    parser = argparse.ArgumentParser(description="Spectrum T-SAC 训练；正式长训练需显式配置预算")
    parser.add_argument("--config")
    parser.add_argument("--mode", choices=("smoke","train","preflight"), default="smoke")
    parser.add_argument("--dataset", help="冻结数据划分清单")
    parser.add_argument("--resume", help="可信的新实现回合边界检查点")
    parser.add_argument("--initialize-from", help="可信CNN检查点的仅权重迁移；新trial，重置优化器/回放/随机流/计数")
    ...
    directory, report = run_training(config, args.mode, args.dataset, args.resume, args.initialize_from)
