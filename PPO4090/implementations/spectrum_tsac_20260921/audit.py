...
from project_paths import PROJECT_ROOT, COVER_OUTPUT_DIR, SPECTRUM_REPORT_DIR
...
def audit_sources():
    ...
    paths += sorted(p for p in (PROJECT_ROOT / "outputs").rglob("*")
                    if p.is_file() and "spectrum_tsac_20260921" not in p.parts)
    for path in paths:
...
def freeze():
    ...
    path = SPECTRUM_REPORT_DIR / "基线冻结清单.json"
    if path.exists():
        return verify(path)
...
原 sac_fyh_io、原始CSV、历史产物保持不变；实现新增于 implementations/spectrum_tsac_20260921，算法名称 Spectrum T-SAC。用户交付说明和产物使用中文文件名；技术模块标识稳定。

第一阶段覆盖生成器、原始业务和assignment未发现；目前只能冻结兼容数据的hash划分，不能证明真实母场景独立，不能宣称48–256支持域已覆盖。D0–D3实现接口及审计工具，缺数据部分标明待上游。

## 验证命令

从PPO4090运行 `python -m implementations.spectrum_tsac_20260921.audit --verify`，重新检查所有冻结文件，无需导入旧训练入口。
'''
    (SPECTRUM_REPORT_DIR / "基线冻结记录.md").write_text(note, encoding="utf-8")
    return {"frozen_files": len(result["inventory"]), **result["statistics"]}


def verify(path=None):
    path = path or SPECTRUM_REPORT_DIR / "基线冻结清单.json"
