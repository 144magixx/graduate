...
def create_app(runs_root=None, datasets_root=None, static_root=None):
    from project_paths import SPECTRUM_OUTPUT_DIR, SPECTRUM_DATA_DIR, SPECTRUM_FRONTEND_DIR
    root = Path(runs_root or SPECTRUM_OUTPUT_DIR).resolve()
    data_root = Path(datasets_root or (root if runs_root is not None else SPECTRUM_DATA_DIR)).resolve()
    assets = Path(static_root or SPECTRUM_FRONTEND_DIR / "dist").resolve()
    app = FastAPI(title="Spectrum T-SAC 只读研究看板", version="1.0.0")
    integrity_cache = {}
    file_hash_cache = {}
