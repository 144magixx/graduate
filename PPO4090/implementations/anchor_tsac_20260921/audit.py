...
from project_paths import PROJECT_ROOT, COVER_OUTPUT_DIR, ANCHOR_REPORT_DIR
...
def audit_sources():
    ...
    paths += sorted(p for p in (PROJECT_ROOT / "outputs").rglob("*")
                    if p.is_file() and "anchor_tsac_20260921" not in p.parts)
    for path in paths:
...
def freeze():
    ANCHOR_REPORT_DIR.mkdir(parents=True, exist_ok=True)
    path = ANCHOR_REPORT_DIR / "Anchor基线冻结清单.json"
    if path.exists():
        return verify(path)
...
日期：2026-09-21。依据：纯Transformer回归与渐进重构方案 v4。本工具只做冻结文件hash审计。

## 源码与数据事实

原始源码、CSV和历史产物的逐文件大小/哈希见《Anchor基线冻结清单.json》。本审计不加载历史checkpoint。
...
原 sac_fyh_io、Horizon、Spectrum、原始CSV与历史产物保持不变；新实现位于 implementations/anchor_tsac_20260921，算法名称 Anchor T-SAC。

第一阶段覆盖生成器、原始业务和assignment未发现；目前只能冻结兼容数据的hash划分，不能证明真实母场景独立，不能宣称48–256支持域已覆盖。D0–D3实现接口及审计工具，缺数据部分标明待上游。

## 验证命令

从PPO4090运行 `python -m implementations.anchor_tsac_20260921.audit --verify`，重新检查冻结文件，无需导入旧训练入口。
'''
    (ANCHOR_REPORT_DIR / "Anchor基线冻结记录.md").write_text(note, encoding="utf-8")
    return {"frozen_files": len(result["inventory"]), **result["statistics"]}


def verify(path=None):
    path = path or ANCHOR_REPORT_DIR / "Anchor基线冻结清单.json"
