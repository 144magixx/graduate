...
def create_app(runs_root=None, datasets_root=None, static_root=None):
    from project_paths import ANCHOR_OUTPUT_DIR, ANCHOR_DATA_DIR, ANCHOR_FRONTEND_DIR
    root = Path(runs_root or ANCHOR_OUTPUT_DIR).resolve()
    data_root = Path(datasets_root or (root if runs_root is not None else ANCHOR_DATA_DIR)).resolve()
    assets = Path(static_root or ANCHOR_FRONTEND_DIR / "dist").resolve()
    app = FastAPI(title="Anchor T-SAC 只读研究看板", version="1.0.0")
    integrity_cache = {}
    file_hash_cache = {}
    ...
    def implementation_metadata():
        """Return declared implementation capabilities, never fabricated run results."""
        return {
            "implementation": "anchor_tsac_20260921",
            "algorithm": {
                "id": "anchor_tsac",
                "model": "pure_transformer_14",
                "token_count": 14,
                "d_model": 128,
                "num_heads": 8,
                "num_layers": 2,
            ...
            "observation": {
                "input": "anchor205",
                "adapter_modes": ["legacy_scaled205", "modern205"],
                "adapter_version_source": "run_manifest.observation_adapter_version",
            },
            "lineage_modes": ["from_scratch", "weights_only_not_exact_resume", "exact_resume"],
            "trial_status": "not_run" if not discover() else "runs_available",
            "is_demo": False,
            "read_only": True,
            "note": "此端点描述实现能力。单次运行的观察适配器、谱系、源 hash 和新 trial 仅以运行清单为准。",
        }

    @app.get("/api/v1/implementation")
    @app.get("/api/implementation", include_in_schema=False)
    def implementation():
    ...
    def comparisons(run_ids: str = Query(..., max_length=2000), phase: str = "validation"):
        ...
        if any(row["manifest"].get("is_demo") or row["manifest"].get("mode") == "demo" for row in rows):
            raise HTTPException(422, "Engineering demo runs are excluded from scientific comparisons")
        ...
        try:
            ...
            if error.name != f"{__package__.rsplit('.', 1)[0]}.research_stats":
                raise
            # The read-only ledger remains useful before optional statistical
            # summarisation is packaged.  Do not synthesize a ranking or interval.
            return {"items": rows, "compatible": not differing and not missing,
                    "differing_fields": differing, "missing_fields": missing,
                    "paired_differences": paired, "ranking_available": False,
                    "return_comparable": bool(return_compatible and not differing and not missing),
                    "phase": phase, "statistics": None, "seed_interval_available": False,
                    "note": "统计汇总未随当前实现打包；仅显示已记录的运行和受可比条件约束的场景配对差值。"}
        groups = {}
        for row in rows:
