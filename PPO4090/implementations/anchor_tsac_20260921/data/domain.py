...
import numpy as np
from project_paths import ANCHOR_DATA_DIR, COVER_OUTPUT_DIR, ANCHOR_REPORT_DIR
...
def read_coverage_spec(directory=None):
    ...
    path = Path(directory or ANCHOR_DATA_DIR) / "训练场景覆盖规格.yaml"
    if not path.is_file():
...
def freeze_run_coverage(run_dir, manifest, config, manifest_path=None):
    ...
    directory = Path(manifest_path).resolve().parent if manifest_path else ANCHOR_DATA_DIR
...
def write_legacy_coverage():
    ...
    for filename, payload in (("训练场景覆盖规格.yaml", spec), ("训练场景清单.json", manifest),
        ...
        (ANCHOR_DATA_DIR / filename).write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    ANCHOR_REPORT_DIR.mkdir(parents=True, exist_ok=True)
    (ANCHOR_REPORT_DIR / "Anchor训练场景覆盖审计.md").write_text(
        ...
        "48–256为候选建设域，未获第一阶段可行性确认。数据/规格/缺口位于PPO4090/data/anchor_tsac_20260921。\n\n"
