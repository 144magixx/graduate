...
class Environment:
    ...
    def observe(self):
        ...
        # legacy205 结构相同但物理已升级；不同槽数不能伪装成原205接口。
        legacy = np.zeros(5+2*p.num_slots, np.float32)
        if not self.terminal:
            legacy[:5] = [s.latitude_deg[current]/90, s.longitude_deg[current]/180, s.demand_bps[current]/1e9,
                          s.ground_diameter_deg[current], self.remaining_power_w/6000]
            legacy[5:5+p.num_slots] = np.log1p(interference/noise)
            legacy[5+p.num_slots:] = row
        obs = {"beam_id": s.beam_id.copy(), "beam_static": static, "beam_dynamic": dynamic, "group_id": s.group_id.copy(),
            ...
            "terminal": self.terminal, "legacy205": legacy, "observation_version": e.observation_version}
