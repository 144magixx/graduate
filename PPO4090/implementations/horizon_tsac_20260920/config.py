...
@dataclass
class ModelConfig:
    model_version: str = "horizon_tsac.v1"
    encoder: str = "full_attention"
    actor: str = "conditional"
    critic: str = "joint"
    d_model: int = 128
...
class Config:
    ...
    def validate(self):
        ...
        for name, value in (("num_groups",p.num_groups),("num_slots",p.num_slots),("max_block_length",e.max_block_length),
                            ("d_model",m.d_model),("attention_heads",m.attention_heads),("encoder_layers",m.encoder_layers),
        ...
        if m.encoder not in ("legacy14","summary","full_pool","full_attention","mlp_local","cnn_local") or m.actor not in ("independent","conditional") or m.critic not in ("additive","joint"):
