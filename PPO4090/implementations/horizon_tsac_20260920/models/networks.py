...
class ObservationEncoder(nn.Module):
    """legacy14/summary/full_pool/full_attention 共用输出维度，完整模型无实体ID位置编码。"""
    def __init__(self, config, example):
        ...
        if self.mode not in ("legacy14", "summary", "full_pool", "full_attention", "mlp_local", "cnn_local"):
        ...
        if self.mode in ("mlp_local", "cnn_local"):
