...
class PolicyNet(nn.Module):
    ...
    def __init__(self, n_states, n_hiddens, freq_dim, slots_dim, power_dim,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, 12, d_model))
    ...
    def forward(self, x: torch.Tensor, freq_mask):
        ...
        local = x[:, :1]                      # (B,4)
        power_pool = x[:, 1: 2]
        row   = x[:, 2:102].view(B, 10, 10)                  # (B,100)
        occ   = x[:, 102:202].view(B, 10, 10)                # (B,100)
...
class ValueNet(nn.Module):
    ...
    def __init__(self, n_states, n_hiddens, freq_dim, slots_dim, power_dim,
        ...
        self.power_embed = nn.Linear(1, d_model)
        self.slot_embed  = nn.Linear(20, d_model)
        self.pos_embed   = nn.Parameter(torch.zeros(1, 12, d_model))
...
class SAC:
    ...
    def init_logging(self):
        # 使用正斜杠定义路径，并确保目录存在
        log_dir = get_output_dir("sac_city_token", "logs")
    ...
    def save_model(self):
        model_dir = get_output_dir("sac_city_token", "models")
