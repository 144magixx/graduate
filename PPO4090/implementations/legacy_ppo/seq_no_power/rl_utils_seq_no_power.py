...
import os
from implementations.legacy_ppo.seq_no_power.Environment_seq_no_power import BeamInfo
...
def train_on_policy_agent(env, agent, num_episodes):
    ...
    for episode in range(num_episodes):  # 将训练分为 10 个阶段
        ...
        transition_dict = {'states': [], 'actions': [], 'next_states': [], 'rewards': [], 'dones': [],'old_log_probs': [],'masks':[]}  # 记录当前回合的数据
        for _ in range(10):
            beam_info = BeamInfo()
            ...
            while not done:  # 游戏未结束时继续
                ...
                env_action = [
                    ...
                    10 * np.log10(30)  # power: 0-50W to dB
                ]
        ...
        util = np.count_nonzero(env.satellite_freq_state.get_state())
        occupy = env.get_occupy()
        print(f"Episode {episode}, Reward: {episode_return:.2f}, Satisfaction: {episode_satisfaction:.2f}, actor_loss{actor_loss}, critic_loss{critic_loss},util{util},occupy{occupy},interfere{env.interfere_num},in1000km{env.interfere_in_1000km_num}")
...
def plot_reward(rewards, window_size=20):
    ...
    save_dir = get_output_dir("legacy_seq_no_power", "figures")
