...
from implementations.legacy_ppo.seq_no_power import rl_utils_seq_no_power as rl_utils
from implementations.legacy_ppo.seq_no_power.Environment_seq_no_power import SatelliteFreqState, BeamInfo, Env, CurrentBeamState, beam_num
...
class PolicyNet(torch.nn.Module):
    ...
    def forward(self, x, freq_mask):
        ...
        return (torch.softmax(self.freq_head(x) + freq_mask, dim=-1),
                torch.softmax(self.slots_head(x), dim=-1))  # 输出动作概率分布，使用 softmax 激活
...
class PPO:
    ...
    def init_logging(self):
        # 使用正斜杠定义路径，并确保目录存在
        log_dir = get_output_dir("legacy_seq_no_power", "logs")
    ...
    def log_step(self, episode, info):
        ...
        log_data = {
            ...
            'interfere_in_1000km': info['interfere_in_1000km']
        }
    ...
    def take_action(self, state, mask):
        ...
        freq_logits, slots_logits = self.actor(state, mask)  # 获取动作概率分布
        ...
        return [freq.cpu().item(),
                slots.cpu().item()], log_prob  # 返回动作（整数）
    ...
    def update(self, transition_dict, entropy_coef):
        ...
        for _ in range(self.epochs):
            freq_logits, slots_logits = self.actor(states, masks)  # 获取动作概率分布
            ...
            entropy = (
                    ...
                    freq_dists.entropy().mean() +
                    slots_dists.entropy().mean() 
                    #power_dists.entropy().mean()
            )
...
model_dir = get_output_dir("legacy_seq_no_power", "models")
