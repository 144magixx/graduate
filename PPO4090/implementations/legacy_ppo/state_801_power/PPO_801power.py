...
from implementations.legacy_ppo.state_801_power import rl_utils_801power as rl_utils
from implementations.legacy_ppo.state_801_power.Environment_801_power import SatelliteFreqState, BeamInfo, Env, CurrentBeamState, beam_num
...
class PPO:
    ...
    def init_logging(self):
        # 使用正斜杠定义路径，并确保目录存在
        log_dir = get_output_dir("legacy_state_801_power", "logs")
    ...
    def update(self, transition_dict, entropy_coef):
        ...
        for _ in range(self.epochs):
            ...
            #print(f" freq_dist{freq_dists.probs},\nfreq_dists.entropy(){ freq_dists.entropy()}, \nfreq_dists.entropy().mean(){ freq_dists.entropy().mean()}")
            entropy = (
            ...
            #print(f"原loss{torch.mean(-torch.min(surr1,surr2))},现loss{actor_loss},entropy{entropy_coef * entropy}")
...
model_dir = get_output_dir("legacy_state_801_power", "models")
