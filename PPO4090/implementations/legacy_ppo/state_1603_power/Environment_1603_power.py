...
from project_paths import CHINA_UNIFORM_POINTS_CSV
...
df = pd.read_csv(CHINA_UNIFORM_POINTS_CSV)
...
class SatelliteFreqState:
    ...
    def update_state(self, group, freq, slots, beam_lat, beam_lon):
        """
        更新资源池状态
        :param group: 波束组 (0-7)
        ...
        # 更新频时矩阵，将分配的时隙标记为1
        self.freq_pool_lat[group, freq:freq + slots] += n_lat
        self.freq_pool_lon[group, freq:freq + slots] += n_lon
...
class Env:
    ...
    def get_combined_state(self, beam_state, freq_state):
        ...
        freq_state_vector = freq_state.get_state()
    ...
    def interfere_beams(self, action):
        ...
        # print(conflict_list)
        return conflict_list, interfere_in_range
    ...
    def SGM_power(self, allocate_power, ideal_power):
        ...
        if abs(delta_S_I) > 5:
            sgm_origin=sgm
            sgm = sgm - abs(delta_S_I)/50
            #print(f"理想功率{ideal_power},分配功率{allocate_power},原sgm{sgm_origin},惩罚后{sgm},惩罚{sgm_origin-sgm}")
        return sgm
    ...
    def get_action_mask(self):
        freq_pool = self.satellite_freq_state.freq_pool_real.reshape(8, 100)[self.beam_idx % 8].reshape(10, 10)
    ...
    def step(self, action):
        ...
        beam_lat, beam_lon = self.beam_info['beam_location'][self.beam_idx]
        if not done:
            self.satellite_freq_state.update_state(group, freq, slots, beam_lat, beam_lon)
            self.current_beam_state.update_state()
        ...
        # 干扰检测与功率计算
        conflict_list, interfere_in_range = self.interfere_beams(action)
        ...
        if interfere_flag:
            ...
            if interfere_in_range:
                ...
                interfere_penalty = 0.5
                # print(10)
            else:
                interfere_penalty = 0.1
                # print(1)
        # print(f"interfere_power_info{interfere_power_info}")
