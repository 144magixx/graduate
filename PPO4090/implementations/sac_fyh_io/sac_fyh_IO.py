...
class PolicyNet(nn.Module):
    ...
    def __init__(self, n_states, n_hiddens, freq_dim, slots_dim, power_dim,
        ...
        # -------- 嵌入层 --------
        self.geo_embed = nn.Linear(2, d_model)   # 4 维 → d_model
        self.rate_embed = nn.Linear(1, d_model)
        self.bw_embed = nn.Linear(1, d_model)
        self.power_embed = nn.Linear(1, d_model)
        self.slot_embed  = nn.Linear(20, d_model)   # 2 维 → d_model

        # 可学习位置编码 (101 = 1 cls + 100 slots)
        self.pos_embed = nn.Parameter(torch.zeros(1, 14, d_model))
    ...
    def forward(self, x: torch.Tensor):
        ...
        bw = x[:, 3: 4]
        power_pool = x[:, 4: 5]
        row   = x[:, 5:105].view(B, 10, 10)                  # (B,100)
        occ   = x[:, 105:205].view(B, 10, 10)                # (B,100)

        slot_feats = torch.cat((row, occ), dim=-1)      # (B,100,2)

        tok0 = self.geo_embed(geo).unsqueeze(1)       # (B,1,d)
        tok1 = self.rate_embed(rate).unsqueeze(1)
        tok2 = self.bw_embed(bw).unsqueeze(1)
        tok3 = self.power_embed(power_pool).unsqueeze(1)
        tokN = self.slot_embed(slot_feats)                # (B,100,d)

        tokens = torch.cat([tok0, tok1, tok2, tok3, tokN], dim=1) + self.pos_embed  # (B,101,d)
        ...
        # -------- 3) 三头输出 --------
        freq_logits  = self.freq_head(pooled)
...
class SAC:
    ...
    def init_logging(self):
        # 使用正斜杠定义路径，并确保目录存在
        log_dir = get_output_dir("sac_fyh_io", "logs")
        ...
        with open(self.log_file, 'w', newline='') as f:
            csv.DictWriter(f, fieldnames=self.log_fields).writeheader()
    ...
    def take_action(self, state):  # 输入当前状态 [n_states]
        ...
        freq_logits, slots_logits, power_logits = self.actor(state)  # 获取动作概率分布
        # 构造与输出动作概率相同的概率分布
    ...
    def calc_target(self, rewards, next_states, dones):
        ...
        probs_freq, probs_slots, probs_power = self.actor(next_states)
    ...
    def update(self, transition_dict):
        ...
        dones = torch.tensor(transition_dict['dones'],
                             dtype=torch.float32, device=device).unsqueeze(1)
        # ---------- 2. TD 目标 ----------
        td_target = self.calc_target(rewards, next_states, dones)  # (B,1)
        ...
        pf, ps, pp = self.actor(states)  # (B,10) × 3
    ...
    def save_model(self):
        model_dir = get_output_dir("sac_fyh_io", "models")
