...
from implementations.horizon_tsac_20260920.data.domain import (DomainSampler, coverage_spec, freeze_run_coverage, validate_domain_annotations)
...
from implementations.horizon_tsac_20260920.telemetry.storage import DB_NAME, readonly
...
class DatasetTrainingTests(unittest.TestCase):
    def test_default_uses_frozen_real_domain_labels_without_inventing_roots(self):
        ...
        stored = json.loads((HORIZON_DATA_DIR/"训练场景清单.json").read_text(encoding="utf-8"))
    ...
    def test_default_rejects_domain_or_spec_tampering_even_after_manifest_rehash(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch.object(train, "HORIZON_DATA_DIR", directory):
    ...
    def test_actual_training_publishes_frozen_audit_and_preserves_offline_evidence(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            path, manifest, spec = annotated_fixture(root/"data", config)
            with patch.object(artifacts, "HORIZON_OUTPUT_DIR", root/"runs"), contextlib.redirect_stdout(io.StringIO()):
