...
from implementations.horizon_tsac_20260920 import overnight
...
from implementations.horizon_tsac_20260920.telemetry.storage import create_schema, DB_NAME
...
def test_supervisor_simulated_children_obey_limit_and_report_missing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(overnight, "HORIZON_OUTPUT_DIR", output)
...
def test_evaluation_worker_uses_immutable_checkpoint_and_all_validation(tmp_path, monkeypatch):
    from implementations.horizon_tsac_20260920 import evaluate
    captured = {}
...
def test_failed_child_launch_is_reported_and_does_not_loop(tmp_path, monkeypatch):
    monkeypatch.setattr(overnight, "HORIZON_OUTPUT_DIR", tmp_path/"runs")
