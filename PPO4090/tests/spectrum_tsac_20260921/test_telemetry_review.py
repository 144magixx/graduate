...
from implementations.spectrum_tsac_20260921.telemetry.recorder import Recorder, _commit, export_run
from implementations.spectrum_tsac_20260921.telemetry.storage import DB_NAME, MANIFEST_NAME, create_schema, dumps, readonly, reconstruct


def make_database(folder, identifier="review"):
    from implementations.spectrum_tsac_20260921.config import Config
    from implementations.spectrum_tsac_20260921.env.action import ActionSpec
...
def step_payload(before, after):
    from implementations.spectrum_tsac_20260921.telemetry.storage import state_delta, state_hash
...
class TelemetryReviewTests(unittest.TestCase):
    def test_integrity_hashes_new_files_only_after_closing_read_transaction(self):
        ...
        import implementations.spectrum_tsac_20260921.dashboard.api as api
    ...
    def test_before_hash_mismatch_rolls_back_and_keyframe_corruption_detected(self):
        from implementations.spectrum_tsac_20260921.telemetry.recorder import RecorderError
        with tempfile.TemporaryDirectory() as temp:
    ...
    def test_rollback_leaves_no_partial_event_or_transition(self):
        from implementations.spectrum_tsac_20260921.telemetry.storage import state_delta
        with tempfile.TemporaryDirectory() as temp:
            conn = make_database(Path(temp)/"run")
            with conn:
                _commit(conn, message("start", "episode_started", {"snapshot": snapshot(0)}, episode_id="ep", step_index=0), 20)
...
def benchmark():
    ...
    from project_paths import SPECTRUM_REPORT_DIR
    import platform
    import sys
    ...
    destination = SPECTRUM_REPORT_DIR/"日志查询与写入性能实测.json"
