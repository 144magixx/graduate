...
from implementations.spectrum_tsac_20260921.models import Actor, Critic, ObservationEncoder, collate_observations, masked_distribution
from implementations.spectrum_tsac_20260921.rl import DiscreteSAC, ReplayBuffer, Transition
from implementations.spectrum_tsac_20260921.rl.sac import exact_expectations, temperature_loss
...
def observation(config, beams=3, terminal=False, remaining=20.):
    ...
    obs = dict(beam_static=rng.normal(size=(beams, 8)).astype(np.float32),
        ...
        current_group=0, remaining_power_w=remaining, legacy205=rng.normal(size=5+2*n).astype(np.float32))
